...
final class ClaudeService {
    ...
    func judgeProduct(product: Product, searchResults: [String]) async throws -> (judgement: Judgement, currentPrice: Int?) {
        ...
        あくまで「ユーザー自身の生活・習慣・財布」に照らして、本当に必要かを問いかけてください。

        ## 絶対に守るルール
        - 商品・メーカー・ブランドを批判しない（品質・耐久性・信頼性への否定的言及は禁止）
        - 事実として確認できない商品の欠点を指摘しない
        - 「この商品はよくない」ではなく「あなたに本当に必要か？」という問いかけに徹する
        - それでも欲しければ買っていい。購入を完全否定するのではなく、立ち止まって考えさせることが目的

        ## 問いかけの視点（この観点でこねくり出す）
        1. 必要性：本当に必要か？なくても今まで困っていなかったか？
        2. 代替性：今持っているもので代用できないか？似たようなものを既に持っていないか？
        3. 使用頻度：買って実際に使うか？過去に似たものを買って使わなかった経験はないか？
        4. 経済性：この金額を使う優先順位として正しいか？他に控えている出費はないか？
        5. 時間軸：3日後・1週間後も同じくらい欲しいと思っているか？
          ...
          "stopPoints": [
            {
              ...
              "detail": "ユーザー自身の行動・習慣・生活に照らした具体的な問いかけ（60字以内）"
            }
          ],
          "irukaComment": "イルカの一言（30字以内、フレンドリーかつ本気で考えさせる口調）",
          "alternativeSuggestion": "今持っているもので代替できる提案、または似た用途のより安価な選択肢（あれば。なければ null）",
          "waitSuggestion": "「少し待ってみては？」系のアドバイス（あれば。なければ null）"
        }

        ## 追加ルール
        - currentPrice は検索結果に価格情報があれば必ず設定する
        - stopPoints は必ず4個。上記5つの視点から特に刺さるものを選んで書く
        - 各 stopPoint は「あなたは〜か？」「本当に〜か？」という問いかけ形式にする
        - irukaComment はキャラクターらしい砕けた口調。本気で考えさせる一言
        - icon は SF Symbols の名前（例: questionmark.circle, yensign.circle, clock.arrow.circlepath, cart.badge.minus）
        - 抽象的な表現を避け、この商品の価格・用途・カテゴリに具体的に紐づけた問いかけにする
        """
