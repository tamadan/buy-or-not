...
final class ClaudeService {
    ...
    func judgeProduct(product: Product, searchResults: [String]) async throws -> (judgement: Judgement, currentPrice: Int?) {
        ...
        - stopPoints は必ず4個。下記の切り口から「この商品に適切なもの」だけを選ぶ
          各切り口に使ってよい条件・使ってはいけない条件を必ず守ること

          ┌─────────────┬──────────────────────────────┬──────────────────────────────┐
          │ 切り口       │ 使ってよい商品                │ 使ってはいけない商品          │
          ├─────────────┼──────────────────────────────┼──────────────────────────────┤
          │ 必要性       │ すべての商品                  │ なし                          │
          │ 使用場面     │ すべての商品                  │ なし                          │
          │ 時間軸       │ すべての商品                  │ なし                          │
          │ 安価代替     │ 代替手段が存在しうるもの      │ 代替不可能な特定用途品        │
          │ 試してから買う│ 体験できる耐久品（家電・家具・│ 食品・消耗品・デジタル        │
          │             │ 衣類・楽器など）かつ3,000円超  │ コンテンツ・低価格品          │
          │ 機会費用     │ 1,000円以上のもの             │ 数百円以下の少額品            │
          └─────────────┴──────────────────────────────┴──────────────────────────────┘
          【適用例】
          肉まん460円   → 試してから買う・機会費用はNG。時間軸・使用場面・安価代替を使う
          AirPods Pro   → すべての切り口が適用可能
          ダイニングテーブル15万円 → 試してから買うが特に有効
        ...
        // 除外した切り口: なし
        {
          ...
          "stopPoints": [
            ...
            { "icon": "hand.raised", "title": "お店で試した？", "detail": "家電量販店でAirPods Pro試聴できるよ。実際に耳に合うか確かめてから買ったほうがいい" }
          ],
        ...
        ### ステップ1：分類コメント（1〜2行）
        // 商品分類: [食品/消耗品/短期耐久品/長期耐久品/サービス/その他]
        // 使用する切り口: [選んだ切り口をカンマ区切りで列挙]
        // 除外した切り口: [除外理由とともに記載。なければ「なし」]
